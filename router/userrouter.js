...
function verifyToken(req, res, next) {
    ...
    if(typeof bearerHeader !== 'undefined') {
      ...
      // Forbidden
      res.json({"statusType":"err","status":"not authorized"})
    }
    ...
    USER.findOne({$or:[{username:req.body.username},{email:req.body.email},{phone:req.body.phone}]},(err,user)=>{
        if(user){
            if(user.username === req.body.username){
                res.json({"statusType":"err","status":"username already exists"})
            }
            else if(user.email === req.body.email){
                res.json({"statusType":"err","status":"email already exists"})
            }
            else if(user.phone === req.body.phone){
                res.json({"statusType":"err","status":"phone already exists"})
            }
        }
        else{
            bcrypt.genSalt(10,(err,salt)=>{
                bcrypt.hash(req.body.password,salt, (err,hash)=>{
                    ...
                    user.save((err)=>{
                        if(err){
                            console.log(err)
                            res.json({"statusType":"err","status":"cannot save try after sometime"})
                        }
                        else{
                            res.json({"statusType":"success","status":"user data saved"})
                        }
                    })
                })
            })
            ...
            bcrypt.compare(req.body.password, user.password, function(err, result) {
                if(result){
                    jwt.sign({user}, 'secretkey', { expiresIn: '5h' }, (err, token) => {
                        res.json({"statusType":"success","status":"logged in","jwt":token})
                    })
                }
                else{
                    res.json({"statusType":"err","status":"incorrect password"})
                }
            })
        }
        else{
            res.json({"statusType":"err","status":"username not found"})
        }
    })
})
